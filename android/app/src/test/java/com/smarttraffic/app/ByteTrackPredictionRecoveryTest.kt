...
class ByteTrackPredictionRecoveryTest {
    ...
    fun handlesVariableTimestampIntervalsWithoutAssumingFixedFrameRate() {
        ...
        val first = tracker.update(listOf(detection(1L, 12f)), 1L, 150L)
        assertEquals(1, first.size)
        assertEquals(1L, first.single().id)

        val second = tracker.update(listOf(detection(2L, 28f)), 2L, 350L)
    ...
    fun rejectsLargeDirectionReversalInsteadOfTeleportingPrediction() {
        val tracker = ByteTrack()
        tracker.update(listOf(detection(0L, 0f)), 0L, 0L)
        tracker.update(listOf(detection(1L, 20f)), 1L, 100L)

        val result = tracker.update(listOf(detection(2L, -90f)), 2L, 200L)
