...
object AutoSpeedGateBuilder {
            ...
            .flatMap { track ->
                track.observations.takeLast(40).asSequence().mapNotNull { observation ->
                    if (calibration == null) {
                        ...
                        Sample((d.left + d.right) * 0.5, d.bottom.toDouble(), observation.timestampMs, track.id)
                    } else {
                        observation.groundPoint?.let { Sample(it.xMeters, it.yMeters, observation.timestampMs, track.id) }
                    }
                }
            }
        ...
        for (track in tracks) {
            ...
            for (i in 1 until observations.size) {
                ...
                if (calibration != null) {
                    ...
                    ax = ((a.detection.left + a.detection.right) * 0.5).toDouble()
                    ay = a.detection.bottom.toDouble()
                    bx = ((b.detection.left + b.detection.right) * 0.5).toDouble()
                    by = b.detection.bottom.toDouble()
                }
                val dx = (bx - ax) / dt
                val dy = (by - ay) / dt
    ...
    private fun pointAtProjection(centerX: Double, centerY: Double, centerProjection: Double, axisX: Double, axisY: Double, targetProjection: Double): Pair<Double, Double> {
...
object SpeedGateEstimator {
    ...
    private fun robustMedianSlope(trajectory: List<Pair<Double, Double>>): Double? {
        ...
        val med = percentile(sorted, 0.5)
        val deviations = sorted.map { abs(it - med) }.sorted()
