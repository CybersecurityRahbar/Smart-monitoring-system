...
fun DashboardScreen(
    ...
    Column(
        modifier = Modifier.padding(paddingValues).padding(horizontal = 18.dp, vertical = 16.dp),
        verticalArrangement = Arrangement.spacedBy(14.dp),
    ) {
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
            Column {
                Text("SMART TRAFFIC", style = MaterialTheme.typography.labelLarge, color = MaterialTheme.colorScheme.primary, letterSpacing = 1.8.sp)
                Text(tr("monitoringCenter"), style = MaterialTheme.typography.headlineMedium)
                Text(tr("situationalAwareness"), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
            AssistChip(onClick = {}, label = { Text(tr("systemOnline")) }, leadingIcon = { Icon(Icons.Filled.CheckCircle, null, Modifier.size(18.dp)) })
        }
            ...
            Column(Modifier.padding(19.dp), verticalArrangement = Arrangement.spacedBy(13.dp)) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Column {
                        ...
                        Text("ESP32-CAM • local network", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                }
                Text(tr("cameraReady"), style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
                Row(horizontalArrangement = Arrangement.spacedBy(9.dp)) {
                    AssistChip(onClick = {}, label = { Text("Open live") })
        ...
        LazyRow(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            items(listOf(Metric(tr("vehicles"), "0", "today"), Metric(tr("averageSpeed"), "—", "km/h"), Metric(tr("activeAlerts"), "0", "active")), key = { it.title }) { metric ->
        ...
        Text("Operational status", style = MaterialTheme.typography.titleMedium)
