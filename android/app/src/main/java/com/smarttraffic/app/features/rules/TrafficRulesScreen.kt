...
private object RulePrefs {
    ...
    fun load(context: Context): Values {
        ...
        return Values(
            limit = p.getInt(LIMIT, 80).toString(),
            warning = p.getInt(WARNING, 70).toString(),
            capture = p.getInt(CAPTURE, 80).toString(),
            confidence = p.getInt(CONFIDENCE, 70).toString(),
    ...
    fun save(context: Context, values: Values) {
        context.getSharedPreferences(NAME, Context.MODE_PRIVATE).edit()
            .putInt(LIMIT, values.limit.toIntOrNull() ?: 80)
            .putInt(WARNING, values.warning.toIntOrNull() ?: 70)
            .putInt(CAPTURE, values.capture.toIntOrNull() ?: 80)
            .putInt(CONFIDENCE, values.confidence.toIntOrNull() ?: 70)
            .putBoolean(ENABLED, values.enabled)
            .apply()
    }

    fun reset(context: Context) {
        save(context, Values("80", "70", "80", "70", true))
    }
}
...
fun TrafficRulesScreen(paddingValues: PaddingValues) {
    ...
    Column(
        Modifier.fillMaxSize().padding(paddingValues).verticalScroll(rememberScrollState()).padding(18.dp),
        ...
        Card(shape = RoundedCornerShape(22.dp)) {
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                ActionRow(tr("captureOnViolation"))
                ActionRow(tr("createAlertOnViolation"))
                ActionRow(tr("preserveEvidence"))
            }
        }
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            Button(
                onClick = {
                    RulePrefs.save(context, Values(speedLimit, warningSpeed, captureSpeed, confidence, enabled))
                    saved = true
                },
        ...
        if (saved) {
            Text(tr("rulesSaved"), style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.primary)
        }
...
private fun RuleField(label: String, value: String, onValueChange: (String) -> Unit, unit: String) {
    OutlinedTextField(
        value = value,
        onValueChange = { text -> if (text.length <= 4 && text.all(Char::isDigit)) onValueChange(text) },
